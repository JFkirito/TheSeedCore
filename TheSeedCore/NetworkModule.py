# -*- coding: utf-8 -*-
"""
TheSeedCore Network Module

Module Description:
This module provides a comprehensive networking service for TheSeedCore, including HTTP server and WebSocket server/client functionalities.
It supports asynchronous operations, secure connections, and customizable message processing.
The module ensures robust error handling and logging for all network operations and is designed to support future expansion.

Main Components:
1. Dependency Check and Import: Checks and imports necessary modules and libraries such as aiohttp and websockets.
2. Logger Setup: Configures a default logger for different network services (HTTP, WebSocket) to handle logging of operations and errors.
3. HTTPServer Class: Implements an HTTP server with configurable routes and handlers.
4. WebSocketServer Class: Implements a WebSocket server for handling real-time communication with clients.
5. WebSocketClient Class: Implements a WebSocket client for connecting to a WebSocket server and handling real-time communication.

Module Functionality Overview:
- Provides an HTTP server with customizable routes and handlers.
- Manages WebSocket servers and clients for real-time communication.
- Supports asynchronous operations and secure connections.
- Allows dynamic configuration and updating of network service parameters.
- Provides detailed logging for monitoring and debugging network operations.
- Ensures compatibility with various network protocols and standards.

Key Classes and Methods:
- _checkDependencies(): Checks and imports module dependencies.
- defaultLogger(): Configures and returns a logger for network services.
- HTTPServer: Core class for managing an HTTP server.
  - _initRoute(): Initializes server routes.
  - checkRoute(): Checks if a route exists.
  - addRoute(): Adds a new route to the server.
  - getHost(): Returns the server host.
  - getPort(): Returns the server port.
  - getServerAddress(): Returns the server address.
  - setAddress(): Sets a new server address.
  - startHTTPServer(): Starts the HTTP server.
  - stopHTTPServer(): Stops the HTTP server.
  - _addressChangedHandler(): Handles address change requests.
- WebSocketServer: Core class for managing a WebSocket server.
  - setMsgProcessor(): Sets the message processing function.
  - setWsProcessor(): Sets the WebSocket processing function.
  - getHost(): Returns the server host.
  - getPort(): Returns the server port.
  - getServerAddress(): Returns the server address.
  ...
  - _wsProcessor(): Default WebSocket connection handler.
  - _registerClient(): Registers a new client.
  - _unregisterClient(): Unregisters a client.
  - _cleanupClient(): Cleans up a client connection.
- WebSocketClient: Core class for managing a WebSocket client.
  - setReceiveProcessor(): Sets the message receive processor.
  - connect(): Connects to the WebSocket server.
  - disconnect(): Disconnects from the WebSocket server.
  - sendMsg(): Sends a message to the WebSocket server.
  - _sendMsgLoop(): Sends messages from the queue.
  - _receiveMsgLoop(): Receives messages from the server.

Notes:
- Ensure all necessary dependencies (aiohttp, websockets, certifi) are installed before using this module.
- Configure network service parameters appropriately in the respective classes.
- Utilize the provided classes to manage HTTP and WebSocket servers and clients.
- Refer to the logging output for detailed information on network operations and errors.
"""
...
_NetWorkServiceSupport = False


def _checkDependencies():
    global _NetWorkServiceSupport
    try:
        import certifi
        import websockets
        from aiohttp import web
        _NetWorkServiceSupport = True
